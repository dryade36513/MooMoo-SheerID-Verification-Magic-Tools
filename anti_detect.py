...
def check_proxy_type(proxy: str) -> str:
    ...
    datacenter_indicators = [
        "vultr", "digitalocean", "linode", "aws", "azure", "gcp",
        "ovh", "hetzner", "contabo", "hostinger"
    ]
    
    residential_indicators = [
        "residential", "mobile", "isp", "roxy", "bright", "oxylabs",
        "smartproxy", "geosurf"
    ]
...
def create_session(proxy: str = None, impersonate: str = None):
    ...
    try:
        ...
        try:
            ...
            print(f"[WARN] Impersonation '{imp_version}' not supported, trying fallback...")
            
            # Try older versions
            for fallback_ver in ["chrome120", "chrome110", "chrome100"]:
                try:
                    if proxies:
                        session = curl_requests.Session(proxies=proxies, impersonate=fallback_ver)
                    else:
                        session = curl_requests.Session(impersonate=fallback_ver)
                    print(f"[Anti-Detect] ✅ Using curl_cffi with {fallback_ver} impersonation")
        ...
        print("[WARN]    Install with: pip install curl_cffi")
        print("[WARN]    Without it, SheerID may detect and reject requests.")
    ...
    if proxies:
        session.proxies = proxies
    print("[Anti-Detect] ❌ Using requests (HIGH detection risk!)")
    print("[Anti-Detect]    Consider installing curl_cffi for better success rate")
...
def warm_session(session, program_id: str = None, headers: dict = None):
    ...
    try:
        ...
        session.get(f"{base_url}/rest/v2/organization/search", params=params, headers=hdrs, timeout=10)
...
def generate_student_email(first_name: str, last_name: str, university: dict = None) -> str:
    """
